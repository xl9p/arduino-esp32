...
public:
  ...
  wl_status_t begin(wifi_init_config_t *config, const char *ssid,
                    const char *passphrase = NULL, int32_t channel = 0,
                    const uint8_t *bssid = NULL, bool connect = true);
  wl_status_t begin(wifi_init_config_t *config, const String &ssid,
                    const String &passphrase = (const char *)NULL,
                    int32_t channel = 0, const uint8_t *bssid = NULL,
                    bool connect = true) {
    return begin(config, ssid.c_str(), passphrase.c_str(), channel, bssid,
                 connect);
  }
